...
pdf(paste("homeoroq_results/ratio_overdispersion_", control, "_vs_", target, ".pdf", sep=""))
...
plot(ctrlRatio[hasTagInCtrl & hasTagInCold], 
     ...
     xlab=paste("A-origin ratio in", control), ylab=paste("A-origin ratio in", target), cex=1.0) # change "cold" to "stress"
par(new=T)
...
pdf(paste("homeoroq_results/hist_overdispersion_", control, "_vs_", target, ".pdf", sep=""))
par(mfrow=c(2,1)) 
